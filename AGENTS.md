...
- Ordinary memory reads accept explicit `maintenance_status=active` and legacy missing status. Quarantined or malformed maintenance records are excluded; `cmd/maintenance analyze` is the content-free inspection path. Viz defaults (including graph and duplicates) are also active-only; authenticated operators may explicitly read valid quarantined ordinary fact summaries (including fact text and point ID) plus normalized maintenance metadata through `GET /viz/api/facts?maintenance_status=quarantined`, with no mutation or purge controls. This protected UI/API is not content-free; CLI manifests, journals, and command output are. Explicit manifest-bound `quarantine`/`restore` require stopped writers plus `--confirm-server-stopped`, produce private content-free journals, and use a bounded cleanup context to preserve ambiguous outcomes after cancellation. Manual purge is CLI-only: explicit manifest IDs, a completed minimum quarantine age, `--confirm-purge`, a fresh snapshot identity re-proved before each deletion, and a checksum-bound mode-`0600` archive outside Qdrant's managed snapshot rotation; no scheduler, and any production run is a separate explicitly approved maintenance window after deployment, never this implementation or its tests. No payload-fingerprint CAS exists in Qdrant: an unverifiable concurrent write is ambiguous, not success.
...
- Schema v4 adds bounded document-routing identity and privacy-safe traces without changing v3 artifacts. `make eval-public-v4` replays hierarchical-only, flat-only, blended RRF, and an explicitly unavailable reranker fail-open case using only isolated Qdrant and precomputed vectors.
- The public v4 routing benchmark has no winner: flat-only improves aggregate ranking but regresses the protected identifier/path cohort, while blended RRF improves aggregate ranking without the required strict protected-cohort improvement. Production remains hierarchical-only. The unavailable reranker replay is resilience evidence only; reranking stays disabled until real model-quality and latency evidence exists.
