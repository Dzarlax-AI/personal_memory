...
- Ordinary memory reads accept explicit `maintenance_status=active` and legacy missing status. Quarantined or malformed maintenance records are excluded; `cmd/maintenance analyze` is the content-free inspection path. Explicit manifest-bound `quarantine`/`restore` require stopped writers plus `--confirm-server-stopped`, produce private content-free journals, and use a bounded cleanup context to preserve ambiguous outcomes after cancellation. No payload-fingerprint CAS exists in Qdrant: an unverifiable concurrent write is ambiguous, not success.
