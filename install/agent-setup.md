...
stops for a reconnect before installing the client policy bundle.
...
configured. Completion requires a fresh client session to discover every
required Memory tool and successfully call `get_stats`.
...
8. Resolve the newest stable GitHub release, or the exact stable `vX.Y.Z`
   requested by the user. Confirm it equals `@RELEASE_VERSION@`. From this
   point every URL must use
...
- the `personal-memory` MCP entry and Codex user configuration or Claude Code
  user scope;
- the fact that Session 1 must stop at `awaiting_reconnect`.
...
Only the exact response `ok` advances the checkpoint to `service_ready`. If the
bounded wait or health check fails, retain `embeddings_ready`, show
`docker compose ps` and the last 50 `memory-mcp` log lines, and stop.

### 6. Register the client through its CLI

First repeat the conflict check. Then execute exactly one matching command:
...
## Session 2: discovery, bundle, and read check
...
- `docker compose ps` and exact `/health` response;
...
Only after real discovery, run the matching commands from the installation
directory, using the actual downloaded binary name and existing real client
root:
...
exist and must not be a symlink. A failed or ambiguous bundle operation is
incomplete and must not be blindly retried.

Finally, call the discovered `get_stats` tool through this client session. Do
not create a sample fact. Only successful quick install, quick verify, and
client-visible `get_stats` permit an atomic transition to `complete`.
...
General diagnostics are limited to `docker compose ps`, the last 50 relevant
