...
Three Docker services: `memory-embeddings`, `memory-qdrant`, `memory-mcp`. The `memory-mcp` container runs all Python services (memory, backup, and optionally todoist + viz) via `entrypoint.py`.
